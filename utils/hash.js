...
/**
 * Хеширование email для дополнительной безопасности
...
export const hashEmail = (email) => {
  try {
    ...
    console.error('Email hash error:', error);
    throw new Error('Ошибка хеширования email');
  }
};
...
/**
 * Хеширование строки (общая функция)
...
export const hashString = (text) => {
  try {
    ...
    const saltedText = text + HASH_KEY;
    return crypto
      .createHash('sha256')
      .update(saltedText)
      .digest('hex');
  } catch (error) {
    console.error('String hash error:', error);
